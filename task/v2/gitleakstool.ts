...
export class GitleaksTool {
  ...
  async getGitLeaksTool(): Promise<string> {
    ...
    if (customtoollocation === undefined) { return await this.getToolFromAgent(specifiedVersion) }
    else { return await this.getToolFromCustomLocation(customtoollocation) }
  }
  ...
  private async findToolVersionOnAgent(version: string): Promise<string | undefined> {
    ...
    if (version.toLowerCase() === 'latest') { return (cachedVersionsbyAgent.sort((one, two) => (one > two ? -1 : 1))).indexOf[0] }
    else { return cachedVersionsbyAgent.find(x => x === toolLib.cleanVersion(version)) }
  }
