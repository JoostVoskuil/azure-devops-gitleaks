...
import { isSemanticVersionGreaterOrEqualThen, getAzureDevOpsInput, getAzureDevOpsVariable, getRequestOptions } from './helpers'
...
export class GitleaksTool {
  ...
  async getGitLeaksTool (): Promise<string> {
    ...
    const customLocation = customtoollocationInput ?? customtoollocationVariabele
    if (customLocation) {
      return await this.getToolFromCustomLocation(customLocation)
    }
  ...
  private getOsArchSuffix (os: string, arch: string, version: string): string | undefined {
    ...
    if (os === 'Windows_NT') {
      ...
      if (lowerArch === 'x86') return 'windows_x32.zip'
    } else if (os === 'Darwin' && lowerArch === 'x64') {
      return 'darwin_x64.tar.gz'
    } else if (os === 'Linux') {
      if (lowerArch === 'x64') return 'linux_x64.tar.gz'
      if (lowerArch === 'x86') return 'linux_x32.tar.gz'
      if (lowerArch === 'arm') return 'linux_armv7.tar.gz'
    }
    return undefined
  }
